...
%%%
%%% @end
%%% Created : 14. 4月 2024 22:41
%%%-------------------------------------------------------------------
-module(binaries_and_bit_syntax).
-author("phiri").
...
%% contd pp102: packing and unpacking 16-bit colors
...
reverse_order_of_bytes_in_a_binary(Data) when is_binary(Data) ->
  {ok, list_to_binary(lists:reverse(binary_to_list(Data)))};
reverse_order_of_bytes_in_a_binary(_) -> {err, data_not_binary }.

term_to_packet(Term) ->
  TermToBytes = term_to_binary(Term), %% use binary_to_term
  <<Header:4,Data>> = TermToBytes,
  {header, Header, data, Data}.

packet_to_term(Packet) ->
  case Packet of
    {header, H, data, D} -> <<H/binary, D/binary>>;
    _ -> invalid_packet

  end.

convert_term() ->
  Converted = term_to_binary({ okay, {person, name,"anderson", age,21}}),
  ByteSize = byte_size(<<1,2,3,4,5,255>>),
  binary_to_term(Converted).
